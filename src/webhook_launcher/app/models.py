...
class WebHookMapping(models.Model):
    ...
    def clean(self, exclude=None):
        ...
        existing = WebHookMapping.objects.filter(project=self.project, package=self.package, obs=self.obs)
        if self.pk:
            existing = existing.exclude(pk=self.pk).filter(masters__in=self.masters.all()).distinct()
        else:
            existing = existing.filter(masters=None)
